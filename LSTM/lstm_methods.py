...
def get_lstm_forecast(train_df, fitted_model, seq_len, n_steps, last_date):
    """
    ...
    target_sequence = train_df[
        -seq_len:
    ]  # последовательность, необходимая для прогноза LSTM

    for _ in range(n_steps):
        X_test = target_sequence[
            -seq_len:
        ]  # последние n значений необходимые для предсказания
    ...
    prediction_dates = pd.date_range(last_date, periods=n_steps + 1, freq=data_freq)[1:]

    # 3. Итоговый DataFrame
    target_sequence = target_sequence[seq_len:]
    pred_df = pd.DataFrame(
        data=target_sequence, index=prediction_dates, columns=["forecast"]
    )
    return pred_df
