...
def get_crammer_values(cm):
    ...
    phi_val_corr = max(
        0, phi_val - ((n_colmns - 1) * (n_rows - 1)) / (n_observations - 1)
    )
...
def get_crammer_values(
    cat_features_df, target, correction=True, return_p_value=True, ascending=False
):
    """
            ...
            if return_p_value:
                ...
                crammer_corrs.append(
                    (
                        ...
                        chi2_res[1],
                        np.sqrt(
                            chi2_res[0] / (n_observations * (min(n_rows, n_colmns) - 1))
                        ),
                    )
                )
            else:
                ...
                crammer_corrs.append(
                    (
                        cat_features_df[feature].name,
                        np.sqrt(
                            chi2_val / (n_observations * (min(n_rows, n_colmns) - 1))
                        ),
                    )
                )
    res_df = pd.DataFrame(
        crammer_corrs, columns=["Feature", "p-value", "V_Crammer_Value"]
...
def math_exp_differences(feat_df, target, ascending=False):
    ...
    """
    main_df = pd.concat(
        [feat_df, target], axis=1
    )  # for combining features with the target
    main_df = (
        main_df.dropna()
    )  # DataFrame must be without NaN values because it affects the result
