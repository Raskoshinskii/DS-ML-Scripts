...
def get_cv_res_class_weights(
    ...
    for weight in log_progress(weights):
        cv_results = cross_validate(
            ...
            n_jobs=-1,
            fit_params={
                'model__sample_weight': y_train.apply(lambda x: weight if x == 1.0 else 1 - weight)
            }
        )
...
def get_cv_results_sampling(
    ...
    n_samples = np.linspace(
        min_class_size, maj_class_size, sample_iters
    )
    n_samples = np.floor(n_samples).astype('int') # get Int values 
    # Split the samples 
    X_train[y_train.name] = y_train
    
    x_train_pos_class = X_train[
        X_train[y_train.name] == max(target_labels)
    ] 
    x_train_neg_class = X_train[
        X_train[y_train.name] == min(target_labels)
    ]
    ...
    if sampling_type == 'under':
        for sample_size in log_progress(n_samples):
            ...
            # Create train data 
            x_train_under = pd.concat(
                [x_train_neg_under, x_train_pos_class],
                axis=0
            )
            ...
            cv_results = cross_validate(
                model, X=x_train_under, y=y_train, 
