...
def cross_validate_pipeline(
    ...
    for model in log_progress(models):
        ...
        cv_results = cross_validate(
            model_pipeline, X=X_train, y=y_train, cv=cv_type, scoring=metrics
        )
...
def make_pipe(
    ...
    num_pipeline = Pipeline(
        [("num_imputing", num_imputer), ("num_scaling", num_scaler)]
    )
    ...
    main_pipeline = Pipeline([("feature_transformations", feature_transformations)])
...
def cross_validate_pipeline(
    ...
    cat_pipeline = Pipeline(
        [("cat_imputing", cat_imputer), ("cat_encoding", cat_encoder)]
    )
    # Numerical Features Processing
    num_pipeline = Pipeline([("data_imputing", num_imputer), ("data_scaling", scaler)])
    transformations = [
        ("num_transformations", num_pipeline, num_columns),
    ...
    for model in log_progress(models):
        model_pipeline = Pipeline(
            [("feature_transformations", feature_transformations), ("model", model)]
        )
        cv_results = cross_validate(
            model_pipeline,
            X=X_train,
