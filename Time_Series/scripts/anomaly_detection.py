...
def double_exponential_smoothing(series, alpha, beta):
    ...
    for n in range(
        1, len(series)
    ):  # add len(series)+1 for forecasting on a single step in the future
        if n == 1:
...
def plot_exponential_smoothing_with_anomalies(
    ...
    exp_smoothing_df = pd.DataFrame(
        exp_smoothing_res, index=series.index
    )  # to see the date on x axis
...
def get_holt_cv_score(
    model_params, series, components_type, metric, season_len, n_splits
):
    """
...
def optimize_holt_hyperparams(
    ...
    print(
        f"Optimal Alpha: {alpha_opt}\nOptimal Beta: {beta_opt}\nOptimal Gamma: {gamma_opt}\n"
    )

    trend, seasonal = components_type
    model = ExponentialSmoothing(
        endog=series, trend=trend, seasonal=seasonal, seasonal_periods=season_len
    )
    model = model.fit(
...
def plot_holt_forecast(
    ...
    plt.axvline(
        train_data.index[-1], ymin=0.05, ymax=0.95, ls="--", color="purple", lw=2.5
    )
    title = (
        " ".join([word.capitalize() for word in metric.__name__.split("_")])
        + " Train"
        + ""
    )
    plt.title(title)
