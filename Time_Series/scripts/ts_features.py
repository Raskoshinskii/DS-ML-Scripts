...
def get_time_features(series, is_cyclical_encoding=False):
    ...
    features_df['is_weekend'] = features_df.index.weekday.isin([5,6])*1 # Myltiple by one to get 1/0 instead if True/False
    ...
    features_df['is_working_hour'] = (
        features_df.reset_index()['tpep_pickup_datetime'].apply(lambda x: x.time() in working_hours)*1
    ).values
    features_df['is_lunch_time'] = (
        features_df.reset_index()['tpep_pickup_datetime'].apply(lambda x: x.time() in lunch_time)*1
    ).values
